...
import { fetchFilmmakerFilms } from "../lib/auth";
...
export function FilmmakerRevenueDashboard({ setView, currentUser }: FilmmakerRevenueDashboardProps) {
  ...
  const [selectedFilmStr, setSelectedFilmStr] = useState<string | null>(null);
  ...
  const revenueByFilm = dbFilms.map((film, index) => ({
    filmId: film.id,
    title: film.title,
    image: film.poster_url || `https://picsum.photos/seed/${film.id}/120/80`,
    status: film.upload_status || "Live",
    totalRevenue: (index + 1) * 10000,
    rentalRevenue: (index + 1) * 5000,
    ownershipRevenue: (index + 1) * 3000,
    collectorRevenue: (index + 1) * 1500,
    secondaryRoyalties: (index + 1) * 500,
    protocolFees: (index + 1) * 500,
    lastTransaction: `${(index + 1) * 3}h ago`,
    split: { director: 40, producer: 30, crew: 20, protocol: 10 },
  }));

  const totalRevenue = revenueByFilm.reduce((sum, f) => sum + f.totalRevenue, 0);
  const selected = revenueByFilm.find((f) => f.filmId === selectedFilmStr) || revenueByFilm[0];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="space-y-4">
            ...
            {revenueByFilm.map((film) => (
              <div
                key={film.filmId}
                onClick={() => setSelectedFilmStr(film.filmId)}
                className={`flex items-center p-4 border cursor-pointer transition-all duration-200 ${
                  (selectedFilmStr === film.filmId || (!selectedFilmStr && film.filmId === revenueByFilm[0].filmId))
